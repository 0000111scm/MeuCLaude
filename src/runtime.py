...
# Mapa de sinônimos PT-BR → tokens que o _score() já entende
SYNONYMS_PTBR: dict[str, list[str]] = {
    "leia":      ["read", "file"],
    "ler":       ["read", "file"],
    "abrir":     ["read", "file"],
    "abra":      ["read", "file"],
    ...
    "rode":      ["bash", "run"],
    "comando":   ["bash", "run"],
    "buscar":    ["glob", "grep", "search"],
    "busque":    ["glob", "grep", "search"],
    "busca":     ["glob", "grep", "search"],
    "listar":    ["glob", "list"],
    "liste":     ["glob", "list"],
    "procurar":  ["grep", "search"],
    "procure":   ["grep", "search"],
    "pesquisar": ["grep", "search"],
    "encontrar": ["grep", "glob", "search"],
    "src":       ["file", "read"],
    "pasta":     ["glob", "directory"],
    "diretorio": ["glob", "directory"],
}
...
class PortRuntime:
    def _expand_tokens(self, tokens: set[str]) -> set[str]:
        """Expande tokens PT-BR para equivalentes em inglês que o _score() reconhece."""
        expanded = set(tokens)
        for token in tokens:
            if token in SYNONYMS_PTBR:
                expanded.update(SYNONYMS_PTBR[token])
        return expanded
    ...
    def route_prompt(self, prompt: str, limit: int = 5) -> list[RoutedMatch]:
        ...
        tokens = self._expand_tokens(raw_tokens)  # ← expansão PT-BR aplicada aqui
